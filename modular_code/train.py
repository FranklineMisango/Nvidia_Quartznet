...
class Trainer:
    def __init__(self, config, rank, world_size, from_checkpoint):
        ...
        if from_checkpoint:
            if os.path.exists(os.path.join(self.checkpoint_dir, "model_last.pt")):
                ...
                with open(
                    os.path.join(self.checkpoint_dir, "last_epoch.txt"), "r"
                ) as f:
                    last_epoch = int(f.read())
                    last_batch_idx = last_epoch * len(self.train_loader) - 1
                    self.start_epoch = last_epoch + 1
                    if self.use_onecyclelr:
                        self.scheduler = self.oneCycleLR(
                            config, last_epoch=last_batch_idx
                        )
            else:
    ...
    def train_step(self, step):
        ...
        for batch_idx, (specs, transcripts, input_lengths, label_length) in enumerate(
            loop
        ):
            ...
            if self.device == 0 or not self.world_size:
                ...
                self.train_writer.add_scalar(
                    f"Epoch {step}: loss", loss, global_step=batch_idx
                )
                ...
                self.train_writer.add_scalar(
                    "Learning Rate",
                    rate,
                    global_step=batch_idx + len(self.train_loader) * (step - 1),
                )

                if batch_idx % 100 == 0:
                    rand_idx = random.randint(0, specs.shape[0] - 1)
                    self.train_writer.add_image(
                        f"Epoch {step} (train): augmented specs",
                        save_spec(specs[rand_idx].to("cpu").detach()),
            ...
            for (
                ...
                if self.device == 0 or not self.world_size:
                    ...
                    self.val_writer.add_text(
                        f"Epoch {step} (val): preds",
                        decoded_preds[rand_idx],
                        global_step=batch_idx,
                    )
                    self.val_writer.add_text(
                        f"Epoch {step} (val): targets",
                        decoded_targets[rand_idx],
                        global_step=batch_idx,
                    )
    ...
    def loader(self, dataset):
        ...
        if self.world_size:
            sampler = DistributedSampler(
                dataset, rank=self.device, num_replicas=self.world_size
            )
            loader = DataLoader(
                dataset,
    ...
    def save_checkpoint(self, path, postfix=""):
        ...
        if self.world_size:
            torch.save(
                ...
                os.path.join(path, f"model_{postfix}.pt"),
            )
        else:
            torch.save(
                self.model.state_dict(), os.path.join(path, f"model_{postfix}.pt")
            )

        torch.save(
            self.optimizer.state_dict(), os.path.join(path, f"optimizer_{postfix}.pt")
        )
    ...
    def load_checkpoint(self, path, map_location):
        ...
        if self.world_size:
            self.model.module.load_state_dict(
                torch.load(
                    os.path.join(path, "model_last.pt"), map_location=map_location
                )
            )
        else:
        ...
        self.optimizer.load_state_dict(
            torch.load(
                os.path.join(path, "optimizer_last.pt"), map_location=map_location
            )
        )
    ...
    def copy_checkpoints(self):
        ...
        shutil.copyfile(
            os.path.join(self.checkpoint_dir, "model_last.pt"),
            os.path.join(self.checkpoint_path, "model_last.pt"),
        )
        shutil.copyfile(
            os.path.join(self.checkpoint_dir, "model_best.pt"),
            os.path.join(self.checkpoint_path, "model_best.pt"),
        )
        shutil.copyfile(
            os.path.join(self.checkpoint_dir, "optimizer_last.pt"),
            os.path.join(self.checkpoint_path, "optimizer_last.pt"),
        )
        shutil.copyfile(
            os.path.join(self.checkpoint_dir, "optimizer_best.pt"),
            os.path.join(self.checkpoint_path, "optimizer_best.pt"),
        )
...
def main():
    ...
    if world_size > 1:
        ...
        trainer = Trainer(
            config, rank=device, world_size=None, from_checkpoint=from_checkpoint
        )
